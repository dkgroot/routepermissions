...
function routepermissions_configpageinit($pagename) {
        ...
        if ($tech_hardware != null || $pagename == 'users') {
                rp_applyhooks();
                $currentcomponent->addprocessfunc('rp_configprocess', 8);
        } elseif ($action=="add") {
...
function rp_configpageload() {
		...
		foreach ($routes as $route) {
			$currentcomponent->addguielem($section, new gui_radio($route, $currentcomponent->getoptlist('rpyn'), rp_get_perm($extdisplay,$route), $route, "" , null));
		}
	}
}
